...
def finalizar_figura(nome_arquivo):
    ...
    if MOSTRAR_GRAFICOS:
        finalizar_figura("velocidade_maxima_vs_amplitude_serie3.png")
    else:
        plt.close()
...
def main():
    ...
    angle = df_plot[COLUNA_ANGULO].to_numpy()
    ...
    amp_peaks_deg, idx_validos = detectar_amplitudes_por_ciclo(
        t_win, angle, t_peaks, distance_samples
    )
    vel_peaks = vel_peaks[idx_validos]

    print(f"--{len(amp_peaks_deg)} amplitudes associadas aos picos de velocidade.")

    plt.figure(figsize=(10, 6))
    plt.plot(amp_peaks_deg, vel_peaks, "o-", markersize=6)

    plt.title("Velocidade máxima vs amplitude — Série 3")
    plt.xlabel("Amplitude (graus)")
    plt.ylabel("Velocidade máxima (rad/s)")
    plt.grid(True, linestyle="--", alpha=0.7)
    finalizar_figura("velocidade_maxima_vs_amplitude_negativa_serie3.png")

    mask_neg = amp_peaks_deg < 0
    mask_pos = amp_peaks_deg >= 0

    if np.any(mask_neg):
        plt.figure(figsize=(10, 6))
        plt.plot(amp_peaks_deg[mask_neg], vel_peaks[mask_neg], "o-", markersize=4)
        plt.title("Velocidade máxima vs amplitude negativa — Série 3")
        plt.xlabel("Amplitude (graus)")
        plt.ylabel("Velocidade máxima (rad/s)")
        plt.grid(True, linestyle="--", alpha=0.7)
        plt.tight_layout()
        finalizar_figura("velocidade_maxima_vs_amplitude_positiva_serie3.png")

    if np.any(mask_pos):
        plt.figure(figsize=(10, 6))
        plt.plot(amp_peaks_deg[mask_pos], vel_peaks[mask_pos], "o-", markersize=4)
        plt.title("Velocidade máxima vs amplitude positiva — Série 3")
        plt.xlabel("Amplitude (graus)")
        plt.ylabel("Velocidade máxima (rad/s)")
        plt.grid(True, linestyle="--", alpha=0.7)
        plt.tight_layout()
        finalizar_figura("GRAFICO_PLACEHOLDER.png")
