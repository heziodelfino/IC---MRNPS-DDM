...
def finalizar_figura(nome_arquivo):
    ...
    if MOSTRAR_GRAFICOS:
        finalizar_figura("velocidade_angular_vs_tempo_serie3.png")
    else:
        plt.close()
...
def main():
    ...
    plt.plot(t_win, signal, label="Velocidade angular", alpha=0.7)
    plt.plot(
        t_peaks,
        vel_peaks,
        "o",
        label=f"Picos detectados (N={len(t_peaks)})",
        markersize=6,
    )

    plt.title(f"Velocidade angular vs tempo — Série 3 ({inicio:.2f}s a {fim:.2f}s)", fontsize=16)
    plt.xlabel("Tempo (s)", fontsize=14)
    plt.ylabel("Velocidade angular (rad/s)", fontsize=14)
