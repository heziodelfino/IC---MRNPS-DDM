...
def finalizar_figura(nome_arquivo):
    ...
    if MOSTRAR_GRAFICOS:
        finalizar_figura("velocidade_maxima_vs_amplitude_loglog_serie3.png")
    else:
        plt.close()
...
def main():
    ...
    print(f"-- {len(x)} pontos válidos para o gráfico log-log.")
    ...
    ax.set_title("Velocidade máxima vs |Amplitude| — Série 3 (log-log)")
    ax.set_xlabel("|Amplitude| (graus)")
    ax.set_ylabel("|Velocidade máxima| (rad/s)")

    finalizar_figura("Vmaxamplitude_loglog.png")
