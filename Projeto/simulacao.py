...
# --- visualizar ajuste ---
t_model = np.linspace(50, 150, 2000)
...
plt.legend()
plt.grid(True)
plt.xlim(50, 150)   # <<< aqui força o eixo do tempo
plt.show()
...
plt.plot(t_model, y_model_fix, '--', color='#3F7EFC', lw=2,
         label=f'Modelo com β encontrado (envelope) = {beta_fix:.5f} 1/s')
plt.xlabel("Tempo (s)")
plt.ylabel("Ângulo (graus)")
plt.title("Fit do pêndulo (50–150 s)")
plt.legend()
