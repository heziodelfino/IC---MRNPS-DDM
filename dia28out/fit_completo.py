#fazer fit completo mostrando os envelopes de cada regime
#"argumento" no qual usamos para fazer dois fits aon invés de 1
...
plt.plot(t_line, -0.00346 * t_line + 4.32466, color="#DC143C",
       label=f'Ajuste linear:\nR = -0,9869', lw=3, linestyle = '--')
plt.plot(t_line, -0.00125 * t_line + 3.79709, color="#20D300",
       label=f'Ajuste linear:\nR= -0,9898', lw=3, linestyle = '--')
plt.xlabel("Tempo (s)", fontsize = 16)
plt.ylabel("ln(Δθ_pico)", fontsize = 16)
plt.xticks(fontsize = 16)
plt.yticks(fontsize = 16)
plt.axvspan(50, 150, color='#A530FF', alpha=0.20, label="1° Regime")
plt.axvspan(150, 1500, color="#FF8D30", alpha=0.20, label="2° Regime")
plt.legend()
plt.grid(True)
plt.show()
